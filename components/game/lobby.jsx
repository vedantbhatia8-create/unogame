...
function ModeCard({ icon, title, desc, active, onClick, accent }) {
  return (
    <button onClick={onClick} style={{
      ...
      background: active ? `${accent}22` : 'rgba(255,255,255,0.04)', transition: 'all .15s',
    }}>
      <div style={{
        width: 46, height: 46, borderRadius: 13, background: accent, flex: '0 0 auto',
        display: 'grid', placeItems: 'center', fontSize: 24, boxShadow: '0 0 0 3px rgba(255,255,255,0.85) inset',
      }}>{icon}</div>
      <div style={{ flex: 1 }}>
...
export function Lobby({ onStart, layout = 'mobile' }) {
  ...
  return (
    <div style={{
      ...
      padding: big ? '48px 20px' : '64px 20px 40px', boxSizing: 'border-box',
      ...
      <div style={{ width: '100%', maxWidth: big ? 460 : 'none' }}>
        ...
        {mode === 'private' && (
          <div style={{
            ...
            <div style={{ fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 13, color: '#0095DA', background: 'rgba(0,149,218,0.15)', padding: '8px 14px', borderRadius: 12 }}>
              Share 🔗
            </div>
          </div>
        )}
        ...
        <button onClick={() => setStack(!stack)} style={{
          ...
          <div style={{ width: 46, height: 27, borderRadius: 14, background: stack ? '#1FA84C' : 'rgba(255,255,255,0.18)', position: 'relative', transition: 'background .15s', flex: '0 0 auto' }}>
            <div style={{ position: 'absolute', top: 3, left: stack ? 22 : 3, width: 21, height: 21, borderRadius: '50%', background: '#fff', transition: 'left .15s', boxShadow: '0 1px 3px rgba(0,0,0,0.3)' }} />
          </div>
        </button>

        <button onClick={() => onStart({ mode, players, stack })} style={{
          width: '100%', marginTop: 22, border: 'none', borderRadius: 18, cursor: 'pointer', padding: '17px',
