...
function Backdrop({ children, blur = true, style = {} }) {
  return (
    <div style={{
      ...
      animation: 'ovFade .22s ease',
      ...style,
    }}>{children}</div>
  );
}

export function WildPicker({ onPick, onCancel, plusFour }) {
  const cells = [
  ...
  return (
    <Backdrop>
      <div style={{ width: '100%', maxWidth: 320, textAlign: 'center' }}>
        <div style={{ fontFamily: "'Fredoka', sans-serif", fontWeight: 600, color: '#fff', fontSize: 24, marginBottom: 4 }}>
          Choose a colour
        </div>
        ...
        {onCancel && (
          <button onClick={onCancel} style={{
            ...
            color: 'rgba(255,255,255,0.55)', fontFamily: "'Nunito', sans-serif", fontWeight: 700, fontSize: 15, cursor: 'pointer',
          }}>Cancel</button>
        )}
      </div>
    </Backdrop>
...
function Confetti() {
  const bits = React.useMemo(() => Array.from({ length: 36 }, (_, i) => ({
    ...
    c: [HEX.red, HEX.yellow, HEX.green, HEX.blue, '#fff'][i % 5],
    rot: Math.random() * 360, size: 7 + Math.random() * 7,
  })), []);
  return (
...
export function WinOverlay({ winner, players, onPlayAgain, onExit, confetti = true }) {
  ...
  return (
    <Backdrop>
      <div style={{ width: '100%', maxWidth: 340, textAlign: 'center' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 18, padding: '8px 14px', textAlign: 'left', marginBottom: 20 }}>
          {ranked.map((p, i) => (
            <div key={p.id} style={{
              ...
              <span style={{ width: 28, height: 28, borderRadius: '50%', background: p.avatar.bg, display: 'grid', placeItems: 'center', fontSize: 15 }}>{p.avatar.glyph}</span>
...
export function PassScreen({ player, onReady }) {
  return (
    <Backdrop blur={false} style={{ background: 'linear-gradient(160deg,#1b1530,#0d0a18)' }}>
      <div style={{ textAlign: 'center', width: '100%', maxWidth: 320 }}>
        <div style={{ color: 'rgba(255,255,255,0.5)', fontFamily: "'Nunito', sans-serif", fontWeight: 800, letterSpacing: 2, fontSize: 13 }}>
          PASS THE DEVICE TO
        </div>
...
export function UnoCallOverlay({ onCall, remaining }) {
  return (
    <div style={{
      ...
      <div style={{ marginTop: 12, height: 6, width: 132, borderRadius: 6, background: 'rgba(255,255,255,0.18)', overflow: 'hidden' }}>
